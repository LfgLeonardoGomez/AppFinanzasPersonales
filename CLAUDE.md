...
45 entradas en 13 fases — índice completo en [`CHANGES.md`](CHANGES.md). **45 archivadas, 0 pendientes — el roadmap está completo.**
...
**El roadmap está completo — sin changes activos ni pendientes de archive.** El último en cerrar fue `C-44` **arquitectura-informacion-ventas-first** (archivado 2026-09-14): housekeeping de arquitectura de información sobre la etapa post-MVP ya cerrada — la home pasa a ser una superficie de acción con "Vender ahora" como protagonista, Estadísticas baja de posición en el menú, y el panel de compras de la ficha de proveedor se retira. Ver el bloque `✅ C-44` más abajo.
...
>
> ✅ **`toFiniteNumber` consolidado el 2026-09-14** (housekeeping, sin change) — vivía duplicado en 8 módulos (no 7: también `actividadRecienteApi.ts`); ahora es un único helper en `src/shared/utils/decimal.ts`. De paso se corrigió un bug real que la duplicación escondía: `cuentaCorrienteApi.ts` y `cuentaCorrienteClienteApi.ts` no rechazaban el string vacío y lo colaban como `0`. Ver `knowledge-base/10_preguntas_abiertas.md`.

> ✅ **C-44 archivado el 2026-09-14 (frontend puro, cero cambios en el backend).** Arquitectura de información, no funcionalidad nueva: la home deja de ser un tablero y pasa a ser una superficie de acción — "Vender ahora" (primaria, hacia `/ventas/nueva`) más la carga con IA conservada; nada de montos, gráficos ni requests HTTP. "Proveedores frecuentes" y "Actividad reciente" se mudan de la home a `/proveedores` (mismos endpoints, `features/home/` se disuelve dentro de `features/proveedores/`). Estadísticas baja del tercer lugar del menú a la posición previa a Perfil — una sola definición de `NAV_ITEMS` alimenta sidebar y barra móvil, así que no hay una segunda lista que pueda divergir. Se retira el panel de compras de la ficha de proveedor (`GET /api/estadisticas/compras` sigue especificado por `estadisticas-backend`, solo pierde su único consumidor). Nueva capability `home-y-navegacion`; deltas `MODIFIED`/`REMOVED` sobre `estadisticas-frontend` y `cuenta-corriente-frontend`. Suite frontend 1035 passed (conteo neto igual: los borrados de los grupos 7/9/10 de `tasks.md` compensan los tests nuevos de los grupos 2-8), `tsc`/`eslint` limpios. Cierre 11.3 por revisión manual del dueño del producto sobre la app corriendo. Detalle en `openspec/changes/archive/2026-09-14-c-44-arquitectura-informacion-ventas-first/design.md` D1-D8.
>
> ⚠️ **Si alguien va a poner un total o un gráfico "chiquito" en la home: no.** Es una restricción activa (D1), no una omisión — el motivo declarado para degradar Estadísticas en el menú es que repite información que ya se ve en otras vistas, y reproducirla en la home cometería el mismo error. La aserción negativa está probada por mutación en `HomePage.test.tsx` (grupo 6.4 de `tasks.md`).
>
> ⚠️ **El saldo de "Proveedores frecuentes" ahora se muestra con el signo de `/proveedores` (deuda en negativo vía `formatSaldo`), no con el de la home vieja** (D4). Es deliberado: dos paneles de la misma pantalla mostrando el saldo del mismo proveedor con signos opuestos no es estética, es una lectura contradictoria del mismo dato — la convención la manda la pantalla destino, no el origen.
