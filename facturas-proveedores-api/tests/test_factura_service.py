...
def _make_pago(session: Session, usuario_id: uuid.UUID, proveedor_id: uuid.UUID, monto: Decimal):
...
class TestCrear:
    ...
    def test_foreign_proveedor_raises_404(self, session: Session):
        ...
        with pytest.raises(HTTPException) as exc_info:
            svc.crear(
                u_a.id,  # user A trying to use user B's supplier
                FacturaCreate(
                    proveedor_id=p_b.id,
                    fecha_emision=date.today(),
    ...
    def test_nuevo_factura_estado_pendiente_when_no_pagos(self, session: Session):
        ...
        result = _create_factura_via_service(session, u.id, p.id, Decimal("100.00"))
        session.commit()
    ...
    def test_nuevo_factura_estado_pagada_when_pool_covers_all(self, session: Session):
        ...
        # Add payment first
        _make_pago(session, u.id, p.id, Decimal("1000.00"))
        session.commit()

        result = _create_factura_via_service(session, u.id, p.id, Decimal("500.00"))
        session.commit()
...
class TestGet:
    def test_returns_factura_with_estado(self, session: Session):
        ...
        u = _make_usuario(session)
        p = _make_proveedor(session, u.id)
        created = _create_factura_via_service(session, u.id, p.id)
        session.commit()

        svc = FacturaService(session)
        result = svc.get(u.id, created.id)
    ...
    def test_foreign_factura_raises_404(self, session: Session):
        ...
        created = _create_factura_via_service(session, u_a.id, p_a.id)
        session.commit()

        svc = FacturaService(session)
        with pytest.raises(HTTPException) as exc_info:
            svc.get(u_b.id, created.id)  # user B tries to access user A's factura
        assert exc_info.value.status_code == 404
    ...
    def test_soft_deleted_raises_404(self, session: Session):
        ...
        svc = FacturaService(session)
        svc.eliminar(u.id, created.id)
        session.commit()

        with pytest.raises(HTTPException) as exc_info:
            svc.get(u.id, created.id)
        assert exc_info.value.status_code == 404
...
class TestListar:
    def test_returns_all_user_facturas_with_estado(self, session: Session):
        ...
        r1 = _create_factura_via_service(session, u.id, p.id, Decimal("100.00"))
        r2 = _create_factura_via_service(session, u.id, p.id, Decimal("200.00"))
        session.commit()

        svc = FacturaService(session)
        results = svc.listar(u.id)
    ...
    def test_estado_filter_applied_in_python_after_fifo(self, session: Session):
        ...
        f1 = _create_factura_via_service(
            session, u.id, p.id, Decimal("100.00"),
            fecha_emision=date.today() - timedelta(days=1),
        )
        f2 = _create_factura_via_service(
            session, u.id, p.id, Decimal("200.00"),
            fecha_emision=date.today(),
        )
        # Pay exactly the first one
        _make_pago(session, u.id, p.id, Decimal("100.00"))
        session.commit()

        svc = FacturaService(session)
        pagadas = svc.listar(u.id, proveedor_id=p.id, estado_filtro=EstadoFactura.PAGADA)
        pendientes = svc.listar(u.id, proveedor_id=p.id, estado_filtro=EstadoFactura.PENDIENTE)
    ...
    def test_user_isolation_in_listar(self, session: Session):
        ...
        f_a = _create_factura_via_service(session, u_a.id, p_a.id)
        session.commit()

        svc = FacturaService(session)
        results_b = svc.listar(u_b.id)
    ...
    def test_fifo_estado_correct_across_multiple_invoices(self, session: Session):
        ...
        f_old = _create_factura_via_service(session, u.id, p.id, Decimal("100.00"), fecha_emision=two_days_ago)
        f_mid = _create_factura_via_service(session, u.id, p.id, Decimal("100.00"), fecha_emision=yesterday)
        f_new = _create_factura_via_service(session, u.id, p.id, Decimal("100.00"), fecha_emision=today)
        _make_pago(session, u.id, p.id, Decimal("150.00"))
        session.commit()

        svc = FacturaService(session)
        results = svc.listar(u.id, proveedor_id=p.id)
...
class TestActualizar:
    def test_partial_update_monto_total(self, session: Session):
        ...
        u = _make_usuario(session)
        p = _make_proveedor(session, u.id)
        created = _create_factura_via_service(session, u.id, p.id, Decimal("100.00"))
        session.commit()
    ...
    def test_foreign_factura_raises_404_on_update(self, session: Session):
        ...
        svc = FacturaService(session)
        with pytest.raises(HTTPException) as exc_info:
            svc.actualizar(u_b.id, created.id, FacturaUpdate(numero="X"))
        assert exc_info.value.status_code == 404
...
class TestEliminar:
    def test_soft_deletes_factura(self, session: Session):
        ...
        svc = FacturaService(session)
        result = svc.eliminar(u.id, created.id)
        session.commit()
    ...
    def test_foreign_factura_raises_404_on_delete(self, session: Session):
        ...
        svc = FacturaService(session)
        with pytest.raises(HTTPException) as exc_info:
            svc.eliminar(u_b.id, created.id)
        assert exc_info.value.status_code == 404
