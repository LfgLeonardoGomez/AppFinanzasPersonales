# negocio-scoping Specification

## Purpose

Define the `Negocio` as the unit of data isolation for the whole system (D-27, shipped by C-28), replacing the per-user scoping the MVP was built on. Every business entity — `Proveedor`, `Factura`, `Pago`, and the Clientes/Ventas tables that follow — is filtered by `negocio_id` in the service layer, so several people can work the same shop from their own accounts and devices while a resource belonging to another shop stays indistinguishable from one that does not exist (404, never 403). This capability owns: the `Negocio` entity and the `Usuario` membership (one user, one negocio, no membership table — `email` is globally unique); how the request's `negocio_id` is resolved (from the hydrated `Usuario`, never a token claim, so a deactivated member loses access on their next request rather than when their token expires); the `es_admin` and `desactivado` flags; the separation between authorship (`creado_por_usuario_id`, informational) and authorization (`negocio_id`, enforced); the reversible data migration that backfills existing rows without changing any resource's effective owner; and public registration creating the `Negocio` and its first admin in a single transaction. Team management on top of these fields — invitations, employee signup, deactivation endpoints and the last-admin guard — belongs to C-29.
