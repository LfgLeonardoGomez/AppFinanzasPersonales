# equipo-backend Specification

## Purpose

Make the team fields that C-28 created actually operable (shipped by C-29). Owns how a person joins a `Negocio` — a single-use, expiring code the admin issues and hands over outside the system, which the employee redeems while choosing their own password, so the admin never handles someone else's credentials — and how a person stops belonging: `desactivado` plus revocation of their active refresh tokens, without deleting a single row they loaded. Also owns the two rules that keep the arrangement safe: a negocio can never be left with no active admin (RN-NEG-08, answered with 409 rather than a misleading 404), and the public redemption endpoint answers identically for unknown, expired and already-used codes so it cannot be used to discover which shops exist. This capability does NOT grant `es_admin`: in C-29 the only administrator of a negocio is whoever created it (D-40), which makes password recovery a practical dependency rather than an optional extra.
