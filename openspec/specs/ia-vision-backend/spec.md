...
Provide the image-only vision-extraction HTTP surface that turns a photographed invoice or payment receipt into a pre-filled Pydantic proposal for the C-09 / C-11 forms, without persisting anything and without ever assigning a supplier. Shipped by C-14, this capability defines the `VisionExtractor` Protocol in `app/services/ia_extraccion_service.py` and ships two implementations — `ClaudeVisionExtractor` (Anthropic SDK) and `OpenAIVisionExtractor` (OpenAI SDK) — plus a `get_vision_extractor()` factory that resolves the implementation from `settings.VISION_PROVIDER`. It exposes two additive endpoints, `POST /api/facturas/extraer-ia` and `POST /api/pagos/extraer-ia`, each accepting multipart image-only uploads (JPEG/PNG/WebP validated by magic bytes, not the `Content-Type` header, ≤ 10 MB; PDFs and other types return 422) and returning a `PropuestaFacturaEnvelope` / `PropuestaPagoEnvelope` whose every field is `Optional` and serializes as `None` when the model could not read it. The capability enforces RN-IA-01 through RN-IA-06 as load-bearing invariants: the IA never persists (a `before_flush` SQLAlchemy listener in the no-persistence regression test asserts zero INSERT/UPDATE/DELETE during the request, RN-IA-04); the IA never assigns a supplier (the proposal returns `proveedor_nombre` as a string and the matching against `Proveedor` is strictly the frontend's `SupplierSearch` / `RN-VINC` job, RN-IA-03 / RN-IA-06); the IA never returns 500 on extractor failure (any SDK exception is captured and the response is 200 with `error: true`, `error_message: str`, and all fields `None`, RN-IA-05); and the rate limiter in `app/core/rate_limit_ia.py` is 10 req/hour per `usuario_id` (stricter than the C-03 per-IP auth limiter because each request spends external API budget, with the 11th request returning 429 + `Retry-After`). The `origen=IA` flag is NOT set here — it is set by the existing manual `POST /api/facturas` / `POST /api/pagos` endpoints when the user confirms the proposal in C-15.
## Requirements
