...
Let someone back into their own account without anyone touching the database (shipped by C-31). It became necessary because of C-29's decision that a negocio has one admin and no promotion path (D-40): a founder who forgets their password would otherwise leave the shop with nobody able to invite or reactivate.

Owns the single-use, one-hour token (hash-only, like refresh tokens and invitations before it — but far shorter-lived, because this one takes over an EXISTING account rather than creating a new one); the public request endpoint that must not reveal who has an account, in its body **or its timing**; applying the new password, which revokes every open session and every other pending token because whoever resets usually believes they were compromised; the rule that an invalid password does not burn the link; and the mail abstraction, whose default console implementation prints to stdout so a developer can actually read the link, and whose real-provider slot fails loudly rather than pretending to send.

Explicitly NOT here: changing a password while logged in (there is a session, so the current password is asked instead), email verification at signup, and second factor.
