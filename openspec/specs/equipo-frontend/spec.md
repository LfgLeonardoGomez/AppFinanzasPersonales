# equipo-frontend Specification

## Purpose

Give the team management that C-29 built a surface (shipped by C-30). Owns the registration screen's two paths — create a negocio versus join one with a code, kept visibly distinct because taking the wrong one produces no error at all, just an employee owning an empty business they believe is their boss's — plus the admin-only team screen: the member list with deactivated people still visible (an admin cannot reactivate who they cannot see), the invitation dialog that refuses to dismiss by backdrop or Escape because the code exists in that one render and nowhere else, and access revocation with its consequences spelled out. Hiding the section from non-admins is courtesy, not access control: the API answers 403 regardless, and this capability never claims otherwise.
