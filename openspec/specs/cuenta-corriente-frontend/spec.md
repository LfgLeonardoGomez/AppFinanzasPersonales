...
Provide the read-only PWA view of the per-supplier cuenta-corriente that consumes the C-12 `GET /api/proveedores/{id}/cuenta-corriente` endpoint without weakening it. Shipped by C-13 as the product's hero screen, this capability exposes a private route `/proveedores/:id` (behind `RequireAuthWithBootstrap`) that renders the on-demand `{ saldo, facturas_con_estado, historial }` triple via a single `useCuentaCorriente(proveedorId)` call: a header with the supplier name (from `useProveedor`, C-07) plus a `SaldoBadge` that color-codes the signed `saldo` (`> 0` deuda red, `= 0` al día green, `< 0` a favor blue); a `TablaFacturasConEstado` that displays the FIFO `estado` from the response field via the existing `EstadoBadge` (C-09) with client-side `estado` / fecha-range filters applied on the response payload (no re-issue, no SQL filter); and a `HistorialCronologico` that renders each row's `saldo_acumulado` from the response, with a visual distinction between `FACTURA` (debe) and `PAGO` (haber) rows. The capability enforces RN-SALDO, RN-FIFO, and RN-HIST as render-only invariants — `saldo`, `estado`, and `saldo_acumulado` are NEVER recomputed on the client; they are read verbatim from the response. Cross-feature cache invalidation is a first-class contract: every `useCreateFactura` / `useUpdateFactura` / `useDeleteFactura` / `useCreatePago` / `useUpdatePago` / `useDeletePago` mutation also invalidates `['cuenta-corriente', 'detail', proveedorId]` for the affected supplier, so a `Cargar factura` or `Cargar pago` action from the detail page triggers a fresh triple on return. Empty / loading / 404 / generic-error states mirror the C-12 error contract (401 handled by the global Axios interceptor, 404 → "Proveedor no encontrado", other → "Reintentar" with a `refetch` button).
## Requirements
