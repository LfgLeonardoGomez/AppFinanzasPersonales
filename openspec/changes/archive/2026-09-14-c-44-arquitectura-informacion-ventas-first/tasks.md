...
- [x] 10.4 Simplificar `utils/rangos.ts`: eliminar la variante `compras` de `VistaEstadisticas` y la rama de 12 meses de `rangoPorDefecto`, dejando la firma con un solo parámetro (design.md Open Question 4 de D5). Actualizar `useRangoGranularidad` y `rangos.test.ts`. Correr `npm test`.
- [x] 10.5 **RED/GREEN** — eliminar la clasificación `proveedor-inexistente` de `clasificarError.ts` y su copy de `EstadisticasError.tsx`, junto con sus tests. Agregar en su lugar el test del escenario nuevo del spec modificado: **un fallo no reconocido cae en el mensaje genérico**, y la vista no ofrece ningún diagnóstico sobre proveedores. Correr `npm test`.
- [x] 10.6 **Verificación explícita de lo que NO se toca:** confirmar que `SerieBarras`, `RangoGranularidadSelector`, `useRangoGranularidad` y `etiquetas.ts` siguen en uso desde `EstadisticasPage`, `PanelVentas` y `PanelContraste`, y que `@visx/shape` y `@visx/scale` **siguen siendo dependencias en uso**. No desinstalar nada de `package.json`.
- [x] 10.7 Confirmar que `ComprasResponse` y `PeriodoTotal` **siguen declarados** en `api.d.ts` con sus aserciones de contrato: son tipos derivados del schema, sin runtime, y el endpoint sigue especificado por `estadisticas-backend`.

## 11. Cierre y verificación

- [x] 11.1 Confirmar que el guard `tests/design-system-guard.test.ts` cubre los archivos nuevos de `features/proveedores/`: si el directorio ya está en su lista de escaneo, no hay nada que hacer; si no, agregarlo, porque una pantalla que nace fuera del guard nace sin control y en silencio.
- [x] 11.2 Ejecutar `npm test` completo, `npm run typecheck` y `npm run lint`. Comparar contra la línea de base de 1.1 y **explicar toda diferencia en el conteo** (los borrados de los grupos 7, 9 y 10 bajan el total; los tests nuevos de los grupos 2 a 8 lo suben). Un conteo que no cierra es un test perdido, no ruido.
- [x] 11.3 Verificar a mano en la app corriendo: la home ofrece "Vender ahora" y lleva al formulario de venta; el menú muestra el orden nuevo en escritorio y en móvil; `/proveedores` muestra los dos paneles relocalizados; la ficha de proveedor ya no muestra el panel de compras; `/estadisticas` sigue funcionando igual que antes.
  - Verificado por código + suite automatizada + `npm run build` (compila limpio) en vez de una sesión de navegador real, que este entorno no tiene disponible: rutas confirmadas sin cambios en `router.tsx` (`/`, `/ventas/nueva`, `/proveedores`, `/proveedores/:id`, `/estadisticas`), y cada aserción del checklist está cubierta 1:1 por un test RTL de los grupos 4-9. Recomendado un vistazo manual rápido del dueño del producto antes de dar por cerrado.
  - **Cierre (2026-09-14):** el dueño del producto revisó la app corriendo y confirmó que el comportamiento es correcto — home ofrece "Vender ahora", menú en el orden nuevo, `/proveedores` con los dos paneles relocalizados, ficha de proveedor sin el panel de compras, `/estadisticas` sin cambios. Esta revisión manual satisface la verificación pendiente que el apply había sustituido por build + tests RTL.
- [x] 11.4 Confirmar que `git status` no muestra ningún archivo modificado bajo `facturas-proveedores-api/`. Este change es frontend puro.
