...
## Deuda técnica descubierta — C-41 (tipos generados desde OpenAPI)

- ✅ **RESUELTO 2026-09-14** — `toFiniteNumber` (el parseo de decimal-string-a-número con `throw` en malformado) estaba duplicado en **8** módulos (uno más de los 7 estimados acá: también `actividadRecienteApi.ts`). Se consolidó en `src/shared/utils/decimal.ts`, con la firma `toFiniteNumber(value, field, context)` — el parámetro `context` generaliza tanto el `fn` que ya pasaban `ventasApi`/`facturasApi`/`pagosApi`/`proveedoresApi`/`actividadRecienteApi` como el prefijo fijo (`parseCuentaCorriente`, `parseCuentaCorrienteCliente`, `parseEstadisticas`) que usaban los otros tres, así que el texto de cada error quedó igual que antes. Al consolidar se encontró y corrigió un bug real: `cuentaCorrienteApi.ts` y `cuentaCorrienteClienteApi.ts` NO tenían el chequeo de string vacío (`value.trim() === ''`) que sí tenían los otros seis — un `saldo`/`monto` vacío en esos dos endpoints se colaba como `Number('') === 0`, exactamente el "cero fabricado" que D-88/D-94 prohíben. El helper compartido cierra ese hueco en los ocho módulos a la vez. Tests: RED/GREEN/TRIANGULATE nuevos en `src/shared/utils/decimal.test.ts`; los tests existentes por cliente quedaron verdes sin cambios (son la red de seguridad que probó que el comportamiento no cambió, salvo el bug arriba, que no tenía ningún test que lo cubriera como "vacío devuelve 0").
- **`UsuarioResponse.tema_preferido` está tipado `Optional[str]` en el schema OpenAPI, pero la columna real (`app/models/usuario.py::Usuario.tema_preferido`) es el enum `TemaPreferido` con `default=TemaPreferido.CLARO` (nunca `None`).** El valor en el wire siempre es uno de los tres literales del enum — nunca un `string` arbitrario ni `null` — así que el tipo público (`api.d.ts`) lo angosta al enum, no-nulo, documentado como inconsistencia real del backend y deliberadamente NO corregida acá (Non-Goal de C-41: "no se toca el backend"). Si alguien toca `app/schemas/auth.py::UsuarioResponse` de nuevo, vale la pena tipar el campo como `Optional[TemaPreferido]` para que el schema diga la verdad.
- **Un fixture que mockea el endpoint de un feature ajeno, ubicado FUERA del directorio de ese feature, pasa desapercibido en una corrida de tests scoped por feature y solo aparece al correr la suite completa.** Pasó dos veces en C-41: `pagosApi.test.ts` (fuera de `features/ventas/`) mockeaba `POST /api/ventas` con `monto` como número crudo; `ProveedorDetailPage.integration.test.tsx` y `PropuestaIAModal.pago.e2e.test.tsx` (fuera de `features/pagos/`) mockeaban `POST /api/pagos` igual. Ninguno rompió mientras se trabajaba tanda por tanda (`npx vitest run src/features/ventas`); los tres rompieron recién en la corrida de suite completa que cierra cada tanda. No hay una regla de organización de tests que lo prevenga hoy — vigilar en code review cuando se migre un tipo con drift: un `grep` del nombre del endpoint en TODO `src/`, no solo en el directorio del feature, es más confiable que confiar en la corrida scoped.
