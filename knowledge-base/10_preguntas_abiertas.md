...
- **`toFiniteNumber` (el parseo de decimal-string-a-número con `throw` en malformado) está duplicado en ~7 módulos.** `proveedoresApi.ts`, `facturasApi.ts`, `pagosApi.ts`, `ventasApi.ts`, `cuentaCorrienteApi.ts`, `cuentaCorrienteClienteApi.ts` y `estadisticasParse.ts` implementan cada uno su propia versión del mismo patrón `Raw*` + `parse*` + lanzar en `NaN` (D-88, D-94). C-41 lo replicó a propósito en cada cliente nuevo en vez de extraer un helper compartido — extraer prematuramente, antes de que los siete casos existieran, habría sido adivinar la forma de una abstracción sin evidencia suficiente. Ahora que los siete existen y son casi idénticos, sí hay evidencia. Candidato razonable para un change de housekeeping chico (`src/shared/utils/decimal.ts` o similar) — no se hizo en C-41 por disciplina de scope (Non-Goal: "no se persigue cobertura de tests nueva más allá de la que el cambio exige", y consolidar es refactor, no derivación de tipos).
