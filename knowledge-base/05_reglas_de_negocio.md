...
| **RN-IA-04** | **Nada se persiste durante la extracción.** El modal de IA no llama a `POST /api/facturas` ni a `POST /api/pagos`; solo abre la propuesta al usuario. La persistencia ocurre **en el form existente** cuando el usuario hace click en "Confirmar" del form (NO del modal). Verificado por test de regresión: la mutación `useCreateFactura`/`useCreatePago` solo se dispara desde el submit del form. |
...
| **RN-IA-06** | **La IA nunca crea ni asigna un proveedor por su cuenta.** La factura/pago no se guarda hasta que el usuario fija un proveedor y confirma. El modal pasa `proveedor_nombre` como string al `SupplierSearch` (C-07), que arranca **sin proveedor seleccionado** aunque la propuesta traiga un nombre. |
| **RN-IA-07** | Rate limit por **`usuario_id`** (no por IP) en `POST /api/facturas/extraer-ia` y `POST /api/pagos/extraer-ia`: **10 requests/hora**. El 11° request responde 429 con header `Retry-After` (segundos hasta que la request más vieja salga de la ventana). El modal muestra countdown y NO auto-retry. |
